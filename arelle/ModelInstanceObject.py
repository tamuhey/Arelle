...
from arelle.ValidateXbrlCalcs import (
    inferredPrecision,
    inferredDecimals,
    roundValue,
    rangeValue,
        ...
        if savedOptions is not None:
            ...
            for fact in xbrlInstance.facts:
                ...
                if (
                    self.entityIdentScheme
                    and self.startDate
                    and self.monetaryUnit
                    and self.monetaryDecimals
                    and self.nonMonetaryDecimals
                ):
                    break
...
class ModelFact(ModelObject):
    ...
    def concept(self):
        ...
        )  # logical (fact) declaration in own modelXbrl, not physical element (if inline)
    ...
    def context(self):
        ...
        try:
            ...
            if not self.modelXbrl.contexts:
                return None  # don't attempt before contexts are loaded
    ...
    def isMultiLanguage(self):
        ...
        return (
            concept is not None
            and concept.type is not None
            and concept.type.isMultiLanguage
        )
    ...
    def ancestorQnames(self):
        ...
        try:
            ...
            self._ancestorQnames = set(
                ModelValue.qname(ancestor) for ancestor in self.iterancestors()
            )
            return self._ancestorQnames
        ...
        try:
            ...
            if decimals:
                ...
                if concept is not None:
                    type = concept.type
                    self._decimals = (
                        type.fixedOrDefaultAttrValue("decimals")
                        if type is not None
                        else None
                    )
                else:
        ...
        try:
            ...
            if precision:
                ...
                if concept is not None:
                    ...
                    self._precision = (
                        type.fixedOrDefaultAttrValue("precision")
                        if type is not None
                        else None
                    )
                else:
    ...
    def xmlLang(self):
        ...
        if isinstance(parentElt, ModelFact):
            ...
            parentElt, ModelObject
        ):  # top-level fact is parented by xbrli:xbrl which isn't a fact
            lang = parentElt.get("{http://www.w3.org/XML/1998/namespace}lang")
            if lang is not None:
                return lang
        # if we got here there is no xml:lang on fact or ancestry
        if (
            self.modelXbrl.modelManager.validateDisclosureSystem
        ):  # use disclosureSystem's defaultXmlLang (if any)
            concept = self.concept
    ...
    def isNil(self, value):
        ...
        if value:
            XmlUtil.setXmlns(
                self.modelDocument, "xsi", "http://www.w3.org/2001/XMLSchema-instance"
            )
        ...
        try:
            ...
            if concept.isNumeric:
                try:
                    ...
                    dec = self.decimals
                    num = roundValue(
                        val, self.precision, dec
                    )  # round using reported decimals
                    if isinf(num):
                        ...
                        if (
                            dec is None or dec == "INF"
                        ):  # show using decimals or reported format
                        ...
                        return Locale.format(
                            self.modelXbrl.locale, "{:.{}f}", (num, dec), True
                        )
                except ValueError:
                    return "(error)"
    ...
    def isVEqualTo(
        ...
        deemP0inf=False,
        normalizeSpace=True,
        numericIntervalConsistency=False,
    ) -> bool:
        ...
        if self.concept.isNumeric:
            if other.concept.isNumeric:
                ...
                if (
                    numericIntervalConsistency
                ):  # values consistent with being rounded from same number
                    (a1, b1, _ia1, _ib2) = rangeValue(
                        self.value, inferredDecimals(self)
                    )
                    (a2, b2, _ia2, _ib2) = rangeValue(
                        other.value, inferredDecimals(other)
                    )
                    return not (b1 < a2 or b2 < a1)

                if (
                    self.modelXbrl.modelManager.validateCalcs
                    != ValidateCalcsMode.XBRL_v2_1_INFER_PRECISION
                ):
                    ...
                    if p == 0:
                        if deemP0Equal:
                            ...
                            return self.xValue == other.xValue
                return roundValue(self.value, precision=p, decimals=d) == roundValue(
                    other.value, precision=p, decimals=d
                )
            else:
                return False
        elif self.concept.isFraction:
            return (
                other.concept.isFraction
                and self.unit is not None
                and self.unit.isEqualTo(other.unit)
                and self.xValue == other.xValue
            )
        selfValue = self.value
        otherValue = other.value
        if (
            normalizeSpace
            and isinstance(selfValue, str)
            and isinstance(otherValue, str)
    ...
    def isDuplicateOf(
        self, other, topLevel=True, deemP0Equal=False, unmatchedFactsStack=None
    ) -> bool:
        ...
        if self.isItem:
            if (
                ...
                or self.parentElement.qname != other.parentElement.qname
            ):
                return False  # can't be identical
            # parent test can only be done if in same instauce
            if (
                self.modelXbrl == other.modelXbrl
                and self.parentElement != other.parentElement
            ):
                return False
            if not (
                self.context.isEqualTo(other.context, dimensionalAspectModel=False)
                and (not self.isNumeric or self.unit.isEqualTo(other.unit))
            ):
                return False
        elif self.isTuple:
            if (
                self == other
                or self.qname != other.qname
                or (topLevel and self.parentElement.qname != other.parentElement.qname)
            ):
                return False  # can't be identical
            ...
            for child1 in self.modelTupleFacts:
                if child1.isItem:
                    if not any(
                        child1.isVEqualTo(child2, deemP0Equal)
                        for child2 in other.modelTupleFacts
                        if child1.qname == child2.qname
                    ):
                        return False
                elif child1.isTuple:
                    if not any(
                        child1.isDuplicateOf(
                            child2, False, deemP0Equal, unmatchedFactsStack
                        )
                        for child2 in other.modelTupleFacts
                    ):
                        return False
    ...
    def md5sum(self):  # note this must work in --skipDTS and streaming modes
        ...
        if self.context is not None:  # not a tuple and has a valid unit
            ...
            _lang = XmlUtil.ancestorOrSelfAttr(
    ...
    def propertyView(self):
        try:
            concept = self.concept
            lbl = (
                ("label", concept.label(lang=self.modelXbrl.modelManager.defaultLang)),
            )
        ...
        return lbl + (
            ...
            + (
                (
                    ...
                    (
                        ("unitRef", unitValue, self.unit.propertyView)
                        if self.isNumeric and self.unit is not None
                        else ()
                    ),
    ...
    def __repr__(self):
        return "modelFact[{0}, qname: {1}, contextRef: {2}, unitRef: {3}, value: {4}, {5}, line {6}]".format(
            self.objectIndex,
            self.qname,
            self.get("contextRef"),
            self.get("unitRef"),
    ...
    def value(self):
        ...
        try:
            ...
            v = XmlUtil.innerText(
                self,
                ixExclude="tuple"
                if self.elementQname == XbrlConst.qnIXbrl11Tuple
                else "html",
                ixEscape=ixEscape,
                ixContinuation=(self.elementQname == XbrlConst.qnIXbrl11NonNumeric),
                ixResolveUris=ixEscape,
                strip=(f is not None),
            )  # transforms are whitespace-collapse, otherwise it is preserved.
            if self.isNil:
                self._ixValue = v
            else:
                if f is not None:
                    if f.namespaceURI in FunctionIxt.ixtNamespaceFunctions:
                        try:
                            v = FunctionIxt.ixtNamespaceFunctions[f.namespaceURI][
                                f.localName
                            ](v)
                ...
                if self.localName == "nonNumeric":
                    ...
                    try:
                        ...
                        self.setInvalid()
                        raise ValueError(
                            "Invalid value for {} number: {}".format(self.localName, v)
                        )
                    try:
                        scale = self.scale
                        ...
                        if isinf(num):
                            ...
                            if num == num.to_integral() and (
                                ".0" not in v or self.isInteger
                            ):
                        ...
                        self.setInvalid()
                        raise ValueError(
                            "Invalid value for {} scale {} for number {}".format(
                                self.localName, scale, v
                            )
                        )
            return self._ixValue
...
class ModelInlineFact(ModelInlineValueObject, ModelFact):
    ...
    def qname(self):
        ...
        try:
            ...
            self._factQname = (
                self.prefixedNameQname(self.get("name")) if self.get("name") else None
            )
            return self._factQname
    ...
    def parentElement(self):
        ...
        return getattr(
            self, "_ixFactParent"
        )  # set by ModelDocument locateFactInTuple for the inline target's root element
    ...
    def propertyView(self):
        if self.localName == "nonFraction" or self.localName == "fraction":
            ...
            numProperties = ()
        return (
            (("file", self.modelDocument.basename), ("line", self.sourceline))
...
class ModelInlineFractionTerm(ModelInlineValueObject, ModelObject):
    ...
    @property
    def concept(self):
        return self.modelXbrl.qnameConcepts.get(
            self.qname
        )  # for fraction term type determination
...
class ModelContext(ModelObject):
    ...
    def init(self, modelDocument):
        ...
        self.segDimValues = {}
        self.scenDimValues = {}
        self.qnameDims: dict[ModelValue.QName, ModelDimensionValue] = {}
        self.errorDimValues = []
    ...
    def isStartEndPeriod(self):
        ...
        try:
            ...
            self._isStartEndPeriod = XmlUtil.hasChild(
                self.period, XbrlConst.xbrli, ("startDate", "endDate")
            )
            return self._isStartEndPeriod
    ...
    def isInstantPeriod(self):
        ...
        try:
            ...
            self._isInstantPeriod = XmlUtil.hasChild(
    ...
    def isForeverPeriod(self):
        ...
        try:
            ...
            self._isForeverPeriod = XmlUtil.hasChild(
                self.period, XbrlConst.xbrli, "forever"
            )
            return self._isForeverPeriod
    ...
    def endDatetime(self):
        ...
        try:
            ...
            self._endDatetime = XmlUtil.datetimeValue(
                XmlUtil.child(self.period, XbrlConst.xbrli, ("endDate", "instant")),
                addOneDay=True,
            )
            return self._endDatetime
    ...
    def instantDatetime(self):
        ...
        try:
            ...
            self._instantDatetime = XmlUtil.datetimeValue(
                XmlUtil.child(self.period, XbrlConst.xbrli, "instant"), addOneDay=True
            )
            return self._instantDatetime
    ...
    def periodHash(self):
        ...
        try:
            ...
            self._periodHash = hash(
                (self.startDatetime, self.endDatetime)
            )  # instant hashes (None, inst), forever hashes (None,None)
            return self._periodHash
        ...
        try:
            ...
            if eiElt is not None:
                self._entityIdentifier = (
                    eiElt.get("scheme"),
                    eiElt.xValue or eiElt.textValue,
                )  # no xValue if --skipDTS
            else:
    ...
    def dimMemberQname(self, dimQname, includeDefaults=False):
        ...
        if (
            dimValue is None
            and includeDefaults
            and dimQname in self.modelXbrl.qnameDimensionDefaults
        ):
    ...
    def contextDimAwareHash(self):
        ...
        try:
            ...
            self._contextDimAwareHash = hash(
                (
                    self.periodHash,
                    self.entityIdentifierHash,
                    self.dimsHash,
                    self.nonDimHash,
                )
            )
    ...
    def contextNonDimAwareHash(self):
        ...
        try:
            ...
            self._contextNonDimAwareHash = hash(
                (
                    self.periodHash,
                    self.entityIdentifierHash,
                    self.segmentHash,
    ...
    def isPeriodEqualTo(self, cntx2):
        ...
        if self.isForeverPeriod:
            ...
            if not cntx2.isStartEndPeriod:
                return False
            return (
                self.startDatetime == cntx2.startDatetime
                and self.endDatetime == cntx2.endDatetime
            )
    ...
    def isEqualTo_(self, cntx2, dimensionalAspectModel):
        ...
        if cntx2 == self:  # same context
            return True
        if (
            self.periodHash != cntx2.periodHash
        ...
        if dimensionalAspectModel:
            ...
            if (
                self.segmentHash != cntx2.segmentHash
                or self.scenarioHash != cntx2.scenarioHash
            ):
                return False
        if (
            self.periodHash != cntx2.periodHash
            or not self.isPeriodEqualTo(cntx2)
            or not self.isEntityIdentifierEqualTo(cntx2)
        ):
            return False
        if dimensionalAspectModel:
            ...
            for dimQname, ctx1Dim in self.qnameDims.items():
                if not ctx1Dim.isEqualTo(cntx2.qnameDims[dimQname]):
                    return False
            for nonDimVals1, nonDimVals2 in (
                (self.segNonDimValues, cntx2.segNonDimValues),
                ...
                if self.isForeverPeriod
                    ...
                    (
                        ...
                        XmlUtil.dateunionValue(
                            self.instantDatetime, subtractOneDay=True
                        ),
                    ),
                )
                if self.isInstantPeriod
                else (
                    ("startDate", XmlUtil.dateunionValue(self.startDatetime)),
                    (
                        "endDate",
                        XmlUtil.dateunionValue(self.endDatetime, subtractOneDay=True),
                    ),
                )
            )
            + (
                (
                    "dimensions",
                    "({0})".format(len(self.qnameDims)),
                    tuple(
                        mem.propertyView for dim, mem in sorted(self.qnameDims.items())
                    ),
                )
                if self.qnameDims
                else (),
    ...
    def __repr__(self):
        return "modelContext[{0}, period: {1}, {2}{3} line {4}]".format(
            self.id,
            "forever"
            if self.isForeverPeriod
            else "instant "
            + XmlUtil.dateunionValue(self.instantDatetime, subtractOneDay=True)
            if self.isInstantPeriod
            else "duration "
            + XmlUtil.dateunionValue(self.startDatetime)
            + " - "
            + XmlUtil.dateunionValue(self.endDatetime, subtractOneDay=True),
...
class ModelDimensionValue(ModelObject):
    ...
    def __hash__(self):
        if self.isExplicit:
            ...
            return hash(
                (
                    self.dimensionQname,
                    XbrlUtil.equalityHash(
                        XmlUtil.child(self), equalMode=XbrlUtil.XPATH_EQ
                    ),
                )
            )
    ...
    def isEqualTo(self, other, equalMode=XbrlUtil.XPATH_EQ):
        ...
        if (
            self.isExplicit
        ):  # other is either ModelDimensionValue or the QName value of explicit dimension
            return self.memberQname == (
                other.memberQname
                if isinstance(other, (ModelDimensionValue, DimValuePrototype))
                else other
            )
    ...
    def propertyView(self):
        if self.isExplicit:
            ...
            return (
                ...
                XmlUtil.xmlstring(
                    XmlUtil.child(self), stripXmlns=True, prettyPrint=True
                ),
            )
    ...
    if parent.xValid >= VALID:  # has DTS and is validated
        return tuple(
            sorted(
                [
                    m.xValue
                    for m in parent.iterchildren(
                        tag="{http://www.xbrl.org/2003/instance}measure"
                    )
        ...
        return tuple(
            sorted(
                [
                    m.prefixedNameQname(m.textValue) or XbrlConst.qnInvalidMeasure
                    for m in parent.iterchildren(
                        tag="{http://www.xbrl.org/2003/instance}measure"
                    )
...
class ModelUnit(ModelObject):
    ...
    def measures(self):
        ...
        try:
            ...
            if self.isDivide:
                self._measures = (
                    measuresOf(
                        XmlUtil.descendant(self, XbrlConst.xbrli, "unitNumerator")
                    ),
                    measuresOf(
                        XmlUtil.descendant(self, XbrlConst.xbrli, "unitDenominator")
                    ),
                )
            else:
    ...
    def md5sum(self):
        try:
            ...
            if self.isDivide:  # hash of mult and div hex strings of hashes of measures
                self._md5sum = md5hash(
                    [md5hash([md5hash(m) for m in md]).toHex() for md in self.measures]
                )
    ...
    def propertyView(self):
        measures = self.measures
        if measures[1]:
            return tuple(("mul", m) for m in measures[0]) + tuple(
                ("div", d) for d in measures[1]
            )
        else:
...
class ModelInlineFootnote(ModelResource):
    ...
    def value(self):
        ...
        try:
            ...
            self._ixValue = XmlUtil.innerText(
                self,
                ixExclude=True,
                ixEscape="html",
    ...
    def xmlLang(self):
        ...
        return XmlUtil.ancestorOrSelfAttr(
            self, "{http://www.w3.org/XML/1998/namespace}lang"
        )
...
class ModelInlineXbrliXbrl(ModelObject):
    ...
    def ixIter(self, childOnly=False):
        ...
        # contexts
        if not hasattr(
            self, "_orderedContenxts"
        ):  # contexts may come from multiple IXDS files
            self._orderedContenxts = sorted(
                modelXbrl.contexts.values(), key=lambda c: c.objectIndex
            )
        ...
        if not hasattr(
            ...
            self._orderedUnits = sorted(
                modelXbrl.units.values(), key=lambda u: u.objectIndex
            )
        ...
        if not hasattr(self, "_orderedFootnoteLinks"):
            ...
            for linkKey, linkPrototypes in modelXbrl.baseSets.items():
                ...
                if (
                    ...
                    and linkqname
                    and arcqname
                    and arcrole != "XBRL-footnotes"  # fully specified roles
                    and any(
            ...
            self._orderedFootnoteLinks = [
                l for r in sorted(_ftLinks.keys()) for l in _ftLinks[r]
            ]
