...
USING_EQUIVALENCE_KEY = sys.intern(
    str("using_equivalence_key")
)  # indicates hash entry replaced with keyed entry
NoneType = type(None)
...
def baseSetArcroles(modelXbrl):
    ...
    return sorted(
        set(
            (XbrlConst.baseSetArcroleLabel(b[0]), b[0])
...
class ModelRelationshipSet:
    __slots__ = (
        ...
        "linkqname",
        "arcqname",
        "modelRelationshipsFrom",
        "modelRelationshipsTo",
        "modelConceptRoots",
        "modellinkRoleUris",
        "modelRelationships",
        "_testHintedLabelLinkrole",
    )
    ...
    def __init__(
        self,
        modelXbrl: ModelXbrl,
        arcrole: Union[str, tuple[str, ...], frozenset[str]],
        linkrole: Union[str, tuple[str, ...], frozenset[str], None] = None,
        linkqname=None,
        arcqname=None,
        includeProhibits=False,
    ):
        ...
        if isinstance(arcrole, (str, NoneType)) and isinstance(
            ...
            for ar in (arcrole,) if isinstance(arcrole, (str, NoneType)) else arcrole:
                for lr in (
                    (linkrole,) if isinstance(linkrole, (str, NoneType)) else linkrole
                ):
                    modelLinks.extend(
                        self.modelXbrl.baseSets.get((ar, lr, linkqname, arcqname), [])
                    )

        # gather arcs
        relationships: dict[Any, ModelRelationship] = {}
        isDimensionRel = (
            self.arcrole == "XBRL-dimensions"
        )  # all dimensional relationship arcroles
        isFormulaRel = (
            self.arcrole == "XBRL-formulae"
        )  # all formula relationship arcroles
        isTableRenderingRel = self.arcrole == "Table-rendering"
        isFootnoteRel = (
            self.arcrole == "XBRL-footnotes"
        )  # all footnote relationship arcroles
        ...
        for modelLink in modelLinks:
            ...
            for linkChild in modelLink:
                ...
                if (
                    linkChild.get("{http://www.w3.org/1999/xlink}type") == "arc"
                    and linkChildArcrole
                ):
                    if (
                        isFootnoteRel
                    ):  # arcrole is fact-footnote or other custom footnote relationship
            ...
            for arcElement in arcs:
                ...
                for fromResource in modelLink.labeledResources[fromLabel]:
                    for toResource in modelLink.labeledResources[toLabel]:
                        if isinstance(
                            fromResource, (ModelResource, LocPrototype)
                        ) and isinstance(toResource, (ModelResource, LocPrototype)):
                            modelRel = ModelDtsObject.ModelRelationship(
                                modelLink.modelDocument,
                                arcElement,
                                fromResource.dereference(),
                            ...
                            if modelRelEquivalenceHash not in relationships:
                                ...
                                if (
                                    otherRel is not USING_EQUIVALENCE_KEY
                                ):  # move equivalentRel to use key instead of hasn
        ...
        orderRels: dict[Any, list[ModelRelationship]] = defaultdict(list)
    ...
    def linkRoleUris(self):
        if self.modellinkRoleUris is None:
            self.modellinkRoleUris = set(
                modelRel.linkrole for modelRel in self.modelRelationships
            )
    ...
    def fromToModelObjects(self, modelFrom, modelTo, checkBothDirections=False):
        ...
        rels = [
            rel
            for rel in self.fromModelObject(modelFrom)
            if rel.toModelObject is modelTo
        ]
        if checkBothDirections:
            rels += [
                rel
                for rel in self.fromModelObject(modelTo)
                if rel.toModelObject is modelFrom
            ]
        return rels
    ...
    def rootConcepts(self):
        if self.modelConceptRoots is None:
            ...
            self.modelConceptRoots = [
                ...
                or (
                    len(relFrom) == 1
                    and len(  # root-level self-looping arc
                        self.modelRelationshipsTo[modelRelFrom]
                    )
                    == 1
                    and relFrom[0].fromModelObject == relFrom[0].toModelObject
                )
            ]
    ...
    def isRelated(
        self, modelFrom, axis, modelTo=None, visited=None, isDRS=False
    ):  # either model concept or qname
        ...
        if isinstance(modelTo, ModelValue.QName):
            ...
            if (
                modelTo is None
            ):  # note that modelTo None (not a bad QName) means to check for any relationship
        ...
        if axis.startswith("ancestral-"):  # allow ancestral-sibling...
            if self.isRelated(
                modelFrom, axis[10:], modelTo
            ):  # any current-level sibling?
                return True
            if visited is None:
                visited = set()
            ...
            isRel = any(
                self.isRelated(
                    modelRel.fromModelObject, axis, modelTo, visited
                )  # any ancestral sibling?
        ...
        if axis.startswith("sibling"):  # allow sibling-or-self or sibling-or-descendant
            ...
            return any(
                self.isRelated(modelRel.fromModelObject, axis, modelTo)
                for modelRel in self.toModelObject(modelFrom)
            )
        for modelRel in self.fromModelObject(modelFrom):
            ...
            if isDescendantAxis:
                ...
                if toConcept not in visited:
                    ...
                    if isDRS:
                        if self.modelXbrl.relationshipSet(
                            consecutiveArcrole[modelRel.arcrole],
    ...
    def label(
        ...
        if linkroleHint:  # order of preference of linkroles to find label
            try:
                ...
                self._testHintedLabelLinkrole = testHintedLinkrole = (
                    len(self.linkRoleUris) > 1
                )
        ...
        for modelLabelRel in labels:
            ...
            if wildRole or role == label.role:
                labelLang = (
                    label.xmlLang
                )  # None if absent or un-declared by empty string (see xml schema)
                if labelLang:
                    labelLang = (
                        labelLang.lower()
                    )  # must be case insensitive for processiing
                ...
                if _lang is None or len(_lang) == 0 or _lang == labelLang:
                    ...
                    if labelLang.startswith(_lang):
                        if not longerLangInLabel or len(longerLangInLabel) > len(
                            labelLang
                        ):
                        ...
                        if not shorterLangInLabel or len(shorterLangInLabel) < len(
                            labelLang
                        ):
